...
- 기존 스냅샷에 없는 신규 타깃을 `New Modules`로 임시 분리하고 모달로 안내
...
- 그래프 위에서 의존 방향 직접 확인
- 현재 그래프 상태를 PNG 이미지로 저장
- 앱 안에서 GitHub Releases 최신 버전 확인 후 바로 다운로드 이동
...
- `양방향`, `내가 의존하는 쪽`, `나에게 의존하는 쪽` 중 방향을 선택합니다.
- 필요하면 깊이를 제한합니다.
...
- 기존 스냅샷에 없는 신규 모듈은 `Pending Review`로 표시되고 `Apply Suggested`로 바로 확정할 수 있습니다.
...
- 스냅샷 파일이 없으면 현재 자동 분류 결과로 초기 스냅샷을 생성합니다.
- 기존 스냅샷이 있으면 저장된 타깃만 먼저 적용하고, 새 타깃은 `New Modules`로 임시 분리합니다.
- `New Modules` 상태의 타깃은 사용자가 확정하기 전까지 스냅샷에 저장되지 않습니다.
...
- 헤더의 `PNG 저장` 버튼을 누르면 현재 그래프를 이미지로 저장할 수 있습니다.
- 저장되는 내용은 현재 보고 있는 그래프 상태 그대로입니다.
  - 기준 모듈
  - 방향 / 깊이 필터
  - 레이어 밴드 배치
  - 현재 활성 경로 강조
  - `Expanded` / `Grouped` 표현 방식
- 파일명은 그래프 이름과 현재 선택 상태를 기준으로 자동 제안됩니다.
- PNG에는 확대/축소 컨트롤 오버레이가 포함되지 않고, 그래프 캔버스만 저장됩니다.

### 8. 앱 업데이트 확인

- 헤더의 `업데이트 확인` 버튼으로 수동 확인이 가능합니다.
- TuistSpider는 GitHub Releases를 기준으로 가벼운 자동 업데이트 체크도 수행합니다.
- 새 버전이 있으면 상단 배너에서 다음 액션을 바로 선택할 수 있습니다.
  - `나중에`
  - `건너뛰기`
  - `다운로드`
- 릴리즈에 DMG 파일이 있으면 그 다운로드 링크를 우선 엽니다.
...
- 기존 스냅샷이 있을 때 스냅샷에 없는 내부 타깃은 적용 레이어 대신 `New Modules`로 분리되고, 추천값은 `Suggested Layer`로만 유지됩니다.
